<template>
  <div class="bvid-convert">
    <div v-if="aid" class="bvid-convert-item">
      {{ aid }}
      <div class="bvid-convert-item-copy" title="复制" @click="copyLink('aid', aid)">
        <VIcon :size="16" :icon="aidCopied ? 'mdi-check' : 'mdi-link'" />
      </div>
    </div>
    <div v-if="aid" class="bvid-convert-item">
      {{ cid }}
      <div class="bvid-convert-item-copy" title="复制" @click="copyLink('cid', cid)">
        <VIcon :size="16" :icon="aidCopied ? 'mdi-check' : 'mdi-link'" />
      </div>
    </div>
    <div v-if="bvid" class="bvid-convert-item">
      {{ bvid }}
      <div class="bvid-convert-item-copy" title="复制" @click="copyLink('bvid', bvid)">
